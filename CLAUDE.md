...
### 3-2. Supabase — 프로젝트는 아직 없고, 스키마는 설계돼 있다

**프로젝트를 만들지 않았다.** `.env.local`도 없다. 따라서 실제 테이블·Storage에 대해 이 문서가 단언하는 내용은 없다.

다만 **스키마는 설계가 끝났고 파일로 있다.**
...
2. `next.config.ts` 이미지 호스트를 **새 프로젝트 도메인으로 교체** (아래 2절 「정리 대상」)
3. `supabase/migrations/0001_products.sql` 실행
4. `docs/dev/schema.md` 맨 위의 「아직 실행되지 않았다」 줄 삭제
