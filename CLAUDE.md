...
- ~~**무엇을 만들어 파는가**~~ → **정해졌다 (2026-09-16). 키링 · 팔찌 · 목걸이 셋이다.** 소재가 아니라 **형태**로 나눈다 — 「비즈」는 재료 이름이었고 이 셋은 몸의 어디에 걸리는지다. 「기타」는 두지 않는다. 헤나타투처럼 형태가 다른 것이 들어오면 **그때 카테고리를 늘린다** — 미리 빈 칸을 만들어 두지 않는다. 근거는 [foundation.md](docs/brand/foundation.md) 8절 1번.
...
- ~~**결제 수단**~~ → **정해졌다 (2026-09-22). 포트원 + 토스페이먼츠 채널, 카드만 받는다.** 옛 사이트도 포트원이었고 그 아래가 KG이니시스였다 — 느렸던 원인이 그쪽일 가능성이 커서 **층은 두고 아래만 토스로 바꿨다.** 설계는 [docs/dev/schema.md 7-4절](docs/dev/schema.md)
...
- 분석: `posthog-js`
- 결제: `@portone/browser-sdk` — **포트원 V2 + 토스페이먼츠 채널.** 서버가 `paymentId` 를 발급하고 결제 뒤 포트원에 직접 물어본다
...
- **쓰기는 관리자에게만 연다 ★** — Supabase Auth 로 로그인하고 RLS 가 이메일을 보고 열어준다(`0003`). 관리자 명단은 `admin_emails` 표에 있고 `is_admin()` 이 판정한다. **secret 키를 쓰지 않는다** — 옛 레포에서 계승한 방식이다. 처음에는 「정책 없이 secret 키로만」이라고 적었는데 그것이 틀렸다

**사진도 Storage 로 옮겼다 (2026-09-19).** 첫 상품 `flower-garden-keyring` 의 사진 5장이 버킷에 있고 화면이 거기서 읽는다. **대시보드에서 올리고 행만 넣으면 배포 없이 상품이 는다** — 이 작업의 목적이 그것이었다.

**상품 사진을 레포에 두지 않는다.** `public/photos/` 에 남은 한 장은 상품이 아니라 「Our Story」 구획의 배경이라 코드가 직접 쓴다. 상품 사진을 다시 레포에 넣지 말 것.

`imageUrl()` 은 `/` 로 시작하는 값과 완성된 주소를 그대로 통과시킨다. 옛 경로가 섞여도 화면이 안 깨지므로 **분기는 남겨둔다.**
...
브랜치 → PR → 머지. **`main`에 직접 push하지 않는다.**

**승인을 기다리지 않는다 (2026-09-22 변경).** 전에는 리뷰 승인 1개가 있어야 머지가 됐는데, 두 사람이 붙어서 만드는 속도에 그것이 맞지 않았다 — 이나래가 자리를 비우면 작업이 그대로 멈췄다.

**그래도 PR 은 반드시 만든다.** 남기는 것이 승인이 아니라 **기록과 알림**이기 때문이다 — `#dev-pr` 로 나가는 「📋 작업 내용」, 「👀 봐주세요」, 그리고 나중에 되짚을 히스토리. 승인만 뺐다.

> **`main`은 GitHub ruleset `main protection`이 지킨다.** 2026-09-12 적용, `enforcement: active`, **bypass list 비어 있음**.
>
> |                             | 무엇이 막아주나                                     |
> | --------------------------- | --------------------------------------------------- |
> | PR 없는 머지                | ruleset `pull_request` — **승인은 0개, PR 은 필수** |
> | `main` 직접 push·force push | ruleset `non_fast_forward` + `pre-push` 훅(로컬)    |
> | `main` 브랜치 삭제          | ruleset `deletion`                                  |
>
> **승인 없는 머지는 이제 막지 않는다.** 2026-09-22에 `required_approving_review_count` 를 1에서 0으로 내렸다. 그전까지는 승인을 기다리느라 작업이 멈췄고, 그때마다 `--admin` 으로 뚫고 싶어졌다 — **규칙을 우회하고 싶어지면 규칙이 틀린 것이다.** 그래서 우회하지 않고 고쳤다.
>
...
| 옵션                    | 무엇                      | 생략하면         |
| ----------------------- | ------------------------- | ---------------- |
| `--changes "..."`       | 바꾼 것. 개발자가 읽는 칸 | 섹션 자체가 빠짐 |
...
**줄바꿈은 네 칸 모두 `\n` 으로 넘긴다** — 작업 내용·`--changes`·`--review`·`--screen`. 문단을 나누려면 `\n\n`, 목록은 `\n- ` 처럼 줄 앞에 붙인다. **특히 「봐주세요」는 ①②③ 을 문단으로 나눠 적는다** — 한 덩어리로 쓰면 리뷰어가 무엇을 봐야 하는지 구분하지 못한다.

**리뷰어는 자동으로 지정된다.** 승인이 없어도 머지되지만 **지정은 계속한다** — 지정 시점에 Slack "리뷰 요청" 알림이 나가고, 그것이 **「이런 것이 들어갔다」를 알리는 유일한 실시간 통로**다. `--no-reviewer` 로 끌 수 있지만 그러면 아무도 모르게 지나간다.
...
**PR 을 만들면 바로 머지할 수 있다.** 승인을 기다리지 않는다. `main`에서 force push·브랜치 삭제는 그대로 금지다.

**대신 「👀 봐주세요」의 성격이 바뀌었다 ★**

전에는 그 칸이 **머지를 막는 질문**이었다. 이제는 **나중에 되짚을 목록**이다. 그래서 적는 법도 달라진다.

|     |                                                               |
| --- | ------------------------------------------------------------- |
| 전  | 「이거 어떻게 할까요?」 — 답을 기다린다                       |
| 후  | **「이렇게 판단했고 근거는 이것이다」** — 읽고 뒤집을 수 있게 |

판단을 미루고 물어보기만 하면 **아무도 안 보는 질문**이 쌓인다. **판단해서 넣고, 왜 그렇게 했는지 남긴다.**

**⚠️ 이 변경으로 잃는 것이 있다.** 2026-09-22 이전 한 주 동안 이나래가 내 실수를 잡아낸 것이 다섯 번이다 — 카카오 동의항목, `0004` 재실행 불가, 장바구니가 사람을 넘어간 것, 이메일 scope 강제, 문서 표 깨짐. **승인이 없어지면 그런 것이 그대로 나간다.**

그래서 **되돌릴 수 없는 것은 여전히 먼저 묻는다** — 마이그레이션 실행, 데이터 삭제, 외부 설정 변경, 그리고 **색과 리듬**(8절). 머지가 쉬워진 것이지 판단이 가벼워진 것이 아니다.
...
### 환경변수는 Type 과 재배포를 같이 본다 ⚠️

Vercel 환경변수에는 **Config** 와 **Secret** 두 타입이 있다. **`NEXT_PUBLIC_` 이 붙었으면 Config 다.** 그 접두사가 「브라우저로 나간다」는 뜻이라 숨길 수가 없고, Secret 으로 넣으면 **값이 빌드에 박히지 않아** 화면에서 「결제 설정이 되어 있지 않습니다」가 뜬다.

**그리고 넣은 뒤 반드시 재배포한다.** `NEXT_PUBLIC_` 값은 빌드할 때 코드에 박히므로 저장만으로는 이미 배포된 것이 바뀌지 않는다.

2026-09-22에 이 둘로 한참 헤맸다. 표와 절차는 [`.env.example`](.env.example) 맨 아래.
...
- **계승한다고 말했으면 원본을 연다 ★** — 디자인과 데이터 모델을 옛 레포([jerryko570/butter-weather-shop](https://github.com/jerryko570/butter-weather-shop), public)에서 계승하기로 했다. 그러면 **관련 작업 전에 그 레포의 해당 파일을 먼저 읽는다.** 기억이나 짐작으로 계승하지 않는다.
  - 2026-09-19 상세 화면에서 두 번 틀렸다 — **상세의 메인 사진이 정사각**인 것을 목록과 같은 3:4로 만들었고(옛 쪽은 목록만 3:4다), **배송·교환 안내가 값까지 이미 정해져 있는데** 「모르는 값이라 지어낼 수 없다」며 통째로 뺐다. 둘 다 원본을 열었으면 안 틀렸다.
  - 특히 **「없으니 지어낼 수 없다」고 판단하기 전에 옛 레포에 있는지부터 본다.** 지어내지 않는 것과 찾아보지 않는 것은 다르다.
- **지금은 첫 화면에서 상품을 보여준다 ★ (2026-09-13)** — foundation.md 5-7절 2번(「첫 화면에서 물건을 팔지 않는다」)은 **이 단계에서 뒤집혔다.** 그 원칙의 근거가 된 브랜드들은 이미 알려진 곳이고 버터웨더는 아니다. **뎁스는 리스크다** — 한 번 더 눌러야 물건이 나오면 그 한 번에서 사람을 잃는다. 룩북은 따로 두지 말고 **상품 그리드 안에 칸으로 끼운다.** 그리고 처음 오는 사람 다수가 한국에서 쇼핑하던 습관을 가지고 오므로 **관습을 깨지 않는다**(왼쪽 카테고리 / 사진 아래 카테고리·이름·가격 / 위쪽 안내 띠). 근거는 foundation.md **5-15절**.
...
지금 위치: **M3 · I3.4 관리자향**이 남았다. 결제까지 돈다.
