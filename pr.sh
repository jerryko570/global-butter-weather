...
#   문단을 나누려면 \n\n. 목록은 \n- 처럼 줄 앞에 붙인다.
#   --review   "<봐주세요>"     판단한 것과 그 근거. 없으면 생략(자동으로 "특별히 없음")
#                               승인이 없어졌으므로 "물어보는 칸"이 아니라
#                               "나중에 되짚을 목록"이다. CLAUDE.md 5절 머지.
...
# 2026-09-22부터 main은 승인을 요구하지 않는다(PR은 여전히 필수).
# 그래도 리뷰어는 계속 지정한다 — 지정 시점에 Slack "리뷰 요청" 알림이
# 나가고, 그것이 "이런 것이 들어갔다"를 알리는 유일한 실시간 통로다.
# --no-reviewer 로 끄면 아무도 모르게 지나간다.
