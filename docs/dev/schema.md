...
5. **`0003_admin_write.sql` 실행** — 아직 안 했다. 관리자 쓰기 정책과 `admin_emails` 표가 들어간다
6. **Supabase Auth 에 관리자 계정을 만든다** — 아직 안 했다. 대시보드 → Authentication → Users → Add user 로 `jerry.narae@gmail.com` 과 `seora0825@gmail.com` 을 만든다.
   **`0003` 이 이메일로 판정하므로 철자가 정확해야 한다.** 표에 없는 이메일로 로그인하면 읽기만 되고 쓰기가 조용히 막힌다.
