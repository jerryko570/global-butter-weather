...
--
--
-- ⚠️ **다시 실행해도 되게 써야 한다.** 각 제약 앞에 `drop constraint if
-- exists` 를 둔 것이 그 때문이다. 처음에는 `add constraint` 만 썼는데,
-- 중간에서 실패하면 **앞쪽은 들어가고 뒤쪽은 안 들어간 상태**로 멈추고
-- 다시 돌리면 「이미 있다」로 거부당한다 (2026-09-19에 실제로 그랬다).
-- 그러면 어디까지 들어갔는지 사람이 세어야 한다.
--
...
alter table products drop constraint if exists products_slug_shape;
...
alter table products drop constraint if exists products_name_en_len;
alter table products
...
alter table products drop constraint if exists products_description_en_len;
alter table products
...
alter table products drop constraint if exists products_images_count;
alter table products
  add constraint products_images_count
    check (array_length(images, 1) is null or array_length(images, 1) <= 10);

alter table products drop constraint if exists products_detail_images_count;
alter table products
...
alter table products drop constraint if exists products_tags_count;
alter table products
...
alter table product_variants drop constraint if exists product_variants_name_len;
alter table product_variants
  add constraint product_variants_name_len
    check (char_length(btrim(name)) between 1 and 40);

alter table product_variants drop constraint if exists product_variants_name_en_len;
alter table product_variants
...
alter table product_variants drop constraint if exists product_variants_price_krw_max;
alter table product_variants
  add constraint product_variants_price_krw_max
    check (price_krw <= 10000000);

alter table product_variants drop constraint if exists product_variants_price_usd_max;
alter table product_variants
  add constraint product_variants_price_usd_max
    check (price_usd is null or price_usd <= 1000000);

alter table product_variants drop constraint if exists product_variants_stock_max;
alter table product_variants
  add constraint product_variants_stock_max
    check (stock <= 9999);

-- position 은 화면 순서다. 음수가 들어가면 정렬이 뒤집힌다
alter table product_variants drop constraint if exists product_variants_position_range;
alter table product_variants
